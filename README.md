...
Основное зеркалирование выполняется через **реальные пользовательские аккаунты** — сообщения в другой мессенджер отправляются от имени самого пользователя (нативное авторство, без префикса `[Имя]:`):
- **Telegram** — MTProto через [Pyrogram](https://docs.pyrogram.org/)
- **MAX** — нативный TCP/SSL бинарный протокол (device_type=DESKTOP)

Дополнительно опционально используются **Telegram-боты** (каждый со своим токеном):
- **MAX→TG bridge bot** — пересылает MAX DM в личку и сообщения от несконфигурированных MAX-пользователей в TG-группы (чтобы они визуально отличались от своих).
- **Admin bot** — удалённое управление бриджем через Telegram.

Боты нужны только для вспомогательных сценариев; само зеркалирование «настроенный пользователь ↔ настроенный пользователь» всегда идёт через пользовательские аккаунты.
...
- `/addbridge`, `/rmbridge` — управление мостами
- `/addbridgeuser`, `/rmbridgeuser` — назначение пользователей в существующий мост
- `/adduser`, `/rmuser` — управление пользователями (реестр)
- `/authmax`, `/authtg` — авторизация аккаунтов через бота
- `/logs [count] [level]` — последние логи
- `/config` — выгрузка текущего config.yaml
- `/restart` — перезапуск бриджа
- `/cancel` — отмена текущего интерактивного сценария
...
**1. DM-бридж** — зеркалирует переписку в MAX DM через личный чат пользователя с этим ботом в Telegram. Односторонний TG DM → MAX не поддерживается: обычные Telegram-DM приходят на ваш TG-аккаунт напрямую, бридж не нужен. А вот MAX DM отражаются в TG-бот чат, и ответить можно через **reply**:

```
  MAX DM (входящее или ваше исходящее из web.max.ru)
        │
        ▼
  [MAX listener обнаруживает сообщение в не-групповом чате]
        │
        ▼
  TG бот → отправляет в ваш личный чат с ботом:
              "[Имя Фамилия]: текст"
              (edit → "[Имя] ✏️: …", delete → "[Имя deleted a message]")
        │
        ▼
  Вы reply'ите на это сообщение в боте
        │
        ▼
  Текст/фото/файл/видео/аудио/voice → отправляется от ВАШЕГО MAX-аккаунта
  в тот же DM-чат (нативное авторство, без префикса)
```

Нюансы:
- MAX доставляет DM-уведомления (opcode 128) только на соединение **отправителя**, поэтому ваши исходящие DM из web.max.ru / мобильного также отражаются в TG-бот чат; входящие DM от собеседников надёжно приходят только если они сами подключены через этот же бридж.
- Reply не-на сообщение → бот отвечает подсказкой. Без reply невозможно определить, в какой MAX DM отправить ответ.
- Удаление в MAX → в TG-бот приходит пометка «[Имя deleted a message]» (само сообщение бота не удаляется, поскольку старый bot-msg_id не связан с пришедшим опкодом удаления).
- Стикеры MAX → «[Sticker]».
- Поддерживаемые медиа в reply из TG-бот → MAX: текст, фото, document, видео, audio, voice (.ogg).
...
- Один бот обслуживает **всех** пользователей из `users` / `bridges`. Каждый пользователь получает свою ветку DM в личке с ботом (по `telegram_user_id`).
- Групповая пересылка: автоопределение доступных TG-групп при старте (если бот в группе → используется; иначе fallback на primary аккаунт с `[Имя]:` префиксом).
- Настройка: создать бота через @BotFather, добавить `max2tg_bridge_bot_token` в `config/credentials.yaml`, каждому пользователю отправить боту `/start` (чтобы TG разрешил боту писать в личку), **добавить бота в нужные TG-группы** для групповой пересылки.
...
| SeenIds (`src/dedup.py`) | Ограниченный FIFO обработанных апдейтов на каждом листенере | Повторная доставка одного и того же апдейта платформой |
...
86 тест-кейсов (65 pass, 6 skip из-за ограничений протокола MAX, 7 manual only, 7 N/A, 1 failed).
...
### `client.unsupported-version` / видео из MAX не пересылается

Обе беды — от одной причины: версии клиента, которой бридж представляется MAX.
Она задана в `src/max/device_profile.py` (`_DEFAULT_APP_VERSION` /
`_DEFAULT_BUILD_NUMBER`) и применяется и при авторизации, и в рантайме.

Если версия устарела, MAX:

* отклоняет **новую** авторизацию с `client.unsupported-version — Приложение
  устарело` (вход по уже выданному токену при этом продолжает работать, так что
  беда всплывает только когда токен протух и нужен `/authmax`);
* выдаёт по `VIDEO_PLAY` (опкод 83) такие ссылки `MP4_480` на OK CDN, которые
  сам CDN отвергает с `400` и телом `10` — подпись при этом валидная. В
  Telegram уходит `[Video — media download failed]`.

Диагностика кодов ответа CDN (подменой параметров в URL): `2` — битая подпись,
`1` — истёкший `expires`, `10` — подпись верна, отказ по другой причине.
Заголовки, `Range`, TLS-отпечаток и хост CDN на это не влияют — проверено.

> ⚠️ **Версию нельзя брать из веб-клиента.** `window.APP_VERSION` на web.max.ru
> — это версия *веб*-линейки, а гейт применяется к *нативной*: 2026-09-04
> веб отдавал `26.9.3`, а нативная авторизация отвергала всё ниже `26.16.1`.
> Раньше эти линейки совпадали настолько, что веб-версия проходила, — сейчас
> уже нет. Ошибиться легко и незаметно: рантайм по живому токену продолжает
> работать, и всё всплывает только на `/authmax`.

**Как починить:** узнать минимально принимаемую версию через хендшейк, потом
прописать её в `device_profile.py` (`_DEFAULT_APP_VERSION`).

Хендшейк (opcode 6) — бесплатный оракул: SMS он не запрашивает, поэтому лимит
авторизаций не тратит. В ответе есть `app-update-type`: `1` — сервер считает
заявленную версию устаревшей, отсутствует или `0` — принимает. По этому полю
версия ищется бинарным поиском:

```python
# connect() + handshake() из src.max.native_client, с MAX_APP_VERSION=<кандидат>
resp["payload"].get("app-update-type")   # 1 = устарела, None/0 = принята
```

Проверено 2026-09-04: гейт смотрит **только на строку `appVersion`**,
`buildNumber` не валидируется (`26.9.3` с build `999999` отвергается,
`99.9.9` с build `18144` проходит). Ставить заведомо завышенную версию не
стоит — та же версия участвует в подписи CDN-ссылок на видео.

Проверить, не трогая код, можно через переменные окружения:

```bash
sudo MAX_APP_VERSION=26.16.1 docker compose \
  -f docker-compose.yml -f docker-compose.server.yml up -d
```

Убедиться, что видео скачивается, помогает встроенная диагностика — она находит
свежие видео в истории бриджуемых чатов, печатает payload `VIDEO_PLAY` и
простукивает все URL:

```bash
sudo VIDEO_PROBE=1 docker compose \
  -f docker-compose.yml -f docker-compose.server.yml up -d
# в логах: PROBE: [MP4_480] → status=200 ctype=video/mp4 len=1882950
```

> ⚠️ `MAX_DEVICE_TYPE` (`DESKTOP`/`ANDROID`/`IOS`) менять на живой сессии
> **нельзя**: токен привязан к типу устройства, объявленному при авторизации, и
> вход отваливается с `FAIL_WRONG_PASSWORD [login.cred]`. Смена типа требует
> полной переавторизации.
...
### Одно сообщение приходит в MAX несколько раз

Причина — повторная доставка апдейта самим Telegram: при деградации соединения
MTProto шлёт неподтверждённые контейнеры апдейтов заново, а Pyrogram (2.0.106)
не помнит уже обработанные и вызывает хендлеры на каждой копии. В логах рядом
видны `Retrying "updates.GetDifference"` и `Connection lost`.

Защита — `SeenIds` в обоих листенерах: ключ `(тип, chat_id, msg_id)` для новых
сообщений и удалений, для правок в ключ добавляется ревизия (время правки +
текст). Дубли отбрасываются со строкой в логе:

```
[bridge.dedup] WARNING: tg:kzolotko: duplicate update ('msg', -639177777, 4242) — dropping
```

Если такие строки появляются часто, стоит смотреть на стабильность
SOCKS-прокси для Telegram, а не на сам мост.
