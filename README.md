...
Самый простой способ: запустить `python -m src.auth` (шаг 4) — после авторизации MAX выводит user ID в логах:
...
| MAX `sender_id=None` | WebSocket возвращает свои же сообщения с `sender_id=None` → skip | Эхо на стороне MAX |
| MirrorTracker | Трекер ID зеркал — `is_max_mirror` / `is_tg_mirror` | Эхо от другого пользователя моста |
...
| Редактирование | ✅ |
| Удаление MAX→TG | ✅ |
| Удаление TG→MAX | ⚠️ только для каналов (Pyrogram не сообщает `chat_id` при удалении в обычных группах) |
